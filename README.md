...
* **How it works:** Downloads or uses a local `.xz` Pi OS image, extracts it, and writes it directly to the target block device (e.g., `/dev/sda`). It then natively mounts the boot and root partitions to inject configurations (including optional VNC enablement) before safely unmounting.
...
* **How it works:** Extracts the source `.xz` image to a raw `.img` file. It uses Linux loopback devices (`losetup`) and `partprobe` to mount the image's internal partitions. It injects configurations (including optional first-boot VNC service) and detaches the loop device, leaving you with a customized `.img` file.

...
* **Advanced Networking (NetworkManager):** Configures multiple Wi-Fi SSIDs with priority tiers.
...
* **VNC Enablement:** Automatically triggers `raspi-config` on first boot to natively enable the VNC service.

---

## 🛠️ The Scripts

### 1. `pi_master_provision.sh` (Direct to Drive)
**Best for:** Standard Linux desktop/server users with an SD card or USB drive plugged in.
* **How it works:** Downloads or uses a local `.xz` Pi OS image, extracts it, and writes it directly to the target block device (e.g., `/dev/sda`). It then natively mounts the boot and root partitions to inject the configurations before safely unmounting.
* **Requirements:** A Linux environment with `sudo` privileges.

### 2. `pi_image_builder.sh` (Offline Image Builder)
**Best for:** Creating reusable, pre-configured `.img` files on a Linux machine to flash later.
* **How it works:** Extracts the source `.xz` image to a raw `.img` file. It uses Linux loopback devices (`losetup`) and `partprobe` to trick the kernel into mounting the image's internal partitions. It injects the files and detaches the loop device, leaving you with a customized `.img` file.
* **Requirements:** A Linux environment with `sudo` privileges and loopback kernel module support.

### 3. `pi_termux_builder.sh` (Termux / Unrooted Android Native)
**Best for:** Mobile users building custom images on Android without root access.
* **How it works:** Because unrooted Android cannot use `mount` or `losetup`, this script operates entirely in user-space. It uses `mtools` to inject configurations directly into the FAT32 boot partition of the `.img` file. It then patches the kernel's `cmdline.txt` with a `systemd.run` hook. On the Pi's very first boot, it executes an injected script, moves all network and SSH configurations into the protected `ext4` root partition natively, enables VNC, and reboots itself.
